...
namespace Stopwatch.ViewModel
{
    ...
    class StopwatchViewModel : INotifyPropertyChanged
    {
        ...
        // Store the last recorded hours value.
        int _lastHours;

        // Store the last recorded minutes value.
        int _lastMinutes;

        // Store the last recorded seconds value with decimal precision.
        decimal _lastSeconds;

        // Store the last recorded running state as a boolean (true or false).
        bool _lastRunning;


        /// <summary>
        /// Event handler for the timer tick event. Updates UI-bound properties when needed.
        /// </summary>
        void TimerTick(object sender, object e)
        {
            // Check if the 'Running' property has changed.
            if (_lastRunning != Running)
            {
                // Update '_lastRunning' with the current 'Running' value.
                _lastRunning = Running;

                // Notify that the 'Running' property has changed.
                OnPropertyChanged("Running");
            }

            // Check if the 'Hours' property has changed.
            if (_lastHours != Hours)
            {
                // Update '_lastHours' with the current 'Hours' value.
                _lastHours = Hours;

                // Notify that the 'Hours' property has changed.
                OnPropertyChanged("Hours");
            }

            // Check if the 'Minutes' property has changed.
            if (_lastMinutes != Minutes)
            {
                // Update '_lastMinutes' with the current 'Minutes' value.
                _lastMinutes = Minutes;

                // Notify that the 'Minutes' property has changed.
                OnPropertyChanged("Minutes");
            }

            // Check if the 'Seconds' property has changed.
            if (_lastSeconds != Seconds)
            {
                // Update '_lastSeconds' with the current 'Seconds' value.
                _lastSeconds = Seconds;

                // Notify that the 'Seconds' property has changed.
